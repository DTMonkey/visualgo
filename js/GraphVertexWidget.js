...
var GraphVertexWidget = function(cx, cy, vertexText, vertexClassNumber){
  ...
  this.transversedVertex = function(){
    var key;

    for(key in graphVertexProperties["innerCircle"]["transversed"]){
      attributeList["innerCircle"][key] = graphVertexProperties["innerCircle"]["transversed"][key];
    }

    for(key in graphVertexProperties["outerCircle"]["transversed"]){
      attributeList["outerCircle"][key] = graphVertexProperties["outerCircle"]["transversed"][key];
    }

    for(key in graphVertexProperties["text"]["transversed"]){
      attributeList["text"][key] = graphVertexProperties["text"]["transversed"][key];
    }
  }
